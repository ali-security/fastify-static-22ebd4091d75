...
function createRoutePrefixMatcher (route) {
  const routePrefix = route.replace(endAsteriskRegex, '')
...
/**
 * The wildcard route param is already percent-decoded by the router, so an
 * encoded path separator (`%2F`) would silently turn into a real separator and
 * escape the guards of any route registered above this plugin. Derive the
 * pathname from the raw url instead and only `decodeURI` it, which leaves the
 * reserved characters — the separators included — encoded.
 * @param {string} url
