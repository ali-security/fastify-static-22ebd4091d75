...
test('does not serve static files with dot-dot path segments', async (t) => {
  ...
  const response = await rawGet(port, '/deep/path/for/test/purpose/foo.html')
  ...
  for (const requestPath of [
    ...
    const dotDotResponse = await rawGet(port, requestPath)
    t.assert.deepStrictEqual(dotDotResponse.statusCode, 403)
    t.assert.match(dotDotResponse.body, /Forbidden/u)
  }

  // the query string is not part of the pathname and must not be inspected
  const queryResponse = await rawGet(port, '/index.html?redirect=/foo/../bar')
  t.assert.deepStrictEqual(queryResponse.statusCode, 200)
  t.assert.deepStrictEqual(queryResponse.body, indexContent)
})
...
test('does not serve static files when dot-dot path segments are consumed by a route param with wildcard `false`', async (t) => {
  t.plan(8)
